...
@JsFun("(url, callback) => { " +
        ...
        "          let speed = f.properties.speed || 0.0;" +         // 🎯 WYCIĄGNIĘCIE SPEED Z PROPERTIES
        "          let timestamp = f.properties.timestamp || 0;" +   // 🎯 WYCIĄGNIĘCIE TIMESTAMP Z PROPERTIES
...
fun setupWebNetwork() {
    ...
    // Wywołanie natywnego fecha JS dla Wasm
    fetchTaborFromJs("http://$acerIp/api/trams/live") { rawData ->
        ...
        if (nowaLista.isNotEmpty()) {
            // 🚀 Aktualizacja w zrefaktoryzowanym TramRepository w warstwie data!
