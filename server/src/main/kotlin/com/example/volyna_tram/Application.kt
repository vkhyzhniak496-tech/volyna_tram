        ...
        route("/api/network/map") {
            ...
            get {
                ...
                if (inputStream != null) {
                    ...
                    call.respondText(
                        "{\"error\": \"Plik grafu nie znaleziony\"}",
                        ContentType.Application.Json
                    )
                }
            }

            // Podścieżka dla peronów: /api/network/map/platforms// Podścieżka dla peronów: /api/network/map/platforms
            get("/platforms") {
                // 1. Ładujemy nowy plik z oczyszczoną geometrią peronów tramwajowych
                val inputStream =
                    this::class.java.classLoader.getResourceAsStream("platforms.geojson")

                if (inputStream != null) {
                    // 2. Czytamy cały GeoJSON z peronami
                    val platformsJsonText = inputStream.bufferedReader().use { it.readText() }

                    // 3. Zwracamy czysty JSON na frontend
                    call.respondText(platformsJsonText, ContentType.Application.Json)
                } else {
                    // Awaryjny fallback, gdyby plik jeszcze nie leżał w resources
                    call.respondText(
                        "{\"error\": \"Plik peronów nie został znaleziony\"}",
                        ContentType.Application.Json
                    )
                }
            }
                ...
                call.respond(
                    listOf(
                        mapOf(
                            "id" to "1",
                            "number" to "26",
                            "start" to "Metro Młociny",
                            "terminus" to "Wiatraczna",
                            "state" to "Early"
                        ),
                        mapOf(
                            "id" to "2",
                            "number" to "19",
                            "start" to "Nowe Bemowo",
                            "terminus" to "Mogiła-Grób",
                            "state" to "On Time"
                        ),
                        mapOf(
                            "id" to "3",
                            "number" to "13",
                            "start" to "Cmentarz Wolski",
                            "terminus" to "Kawęczyńska-Bazylika",
                            "state" to "Delayed"
                        )
                    )
                )
            ...
            route("/api/trams") {
                ...
                get {
                    ...
                    val json = tramwajeBaza.joinToString(
                        prefix = "[\n",
                        postfix = "\n]",
                        separator = ",\n"
                    ) {
                ...
                post {
                    ...
                    if (nowaLinia != null) {
                        ...
                        call.respondText(
                            "{\"message\": \"Dodano pomyślnie linię $nowaLinia\"}",
                            ContentType.Application.Json,
                ...
                route("/{linia}") {
                    ...
                    get {
                        ...
                        if (tramwaj != null) {
                            val json =
                                """{"linia": "${tramwaj.linia}", "trasa": "${tramwaj.trasa}", "status": "${tramwaj.status}"}"""
                            call.respondText(json, ContentType.Application.Json)
                        } else {
                            call.respondText(
                                "{\"error\": \"Nie znaleziono linii $numer\"}",
                                ContentType.Application.Json,
...
// Wywołanie: /api/trams/26?status=Korek&trasa=Metro Młociny - Rondo Żaba
                    patch {
                        ...
                        if (tramwaj != null) {
                            ...
                            if (cosZmieniono) {
                                ...
                                call.respondText(
                                    "{\"error\": \"Brak parametrów do zmiany\"}",
                                    ContentType.Application.Json,
                                    HttpStatusCode.BadRequest
                                )
                            }
                    ...
                    delete {
                        ...
                        if (usunięto) {
                            ...
                            call.respondText(
                                "{\"message\": \"Linia $numer usunięta z systemu\"}",
                                ContentType.Application.Json
                            )
                        } else {
                            call.respondText(
                                "{\"error\": \"Nie ma takiej linii w systemie\"}",
                                ContentType.Application.Json,
                                HttpStatusCode.NotFound
                            )
                        }
